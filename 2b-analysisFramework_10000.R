...
simDatFiles <- list.files("simParts10000", pattern=".*\\.RData", full.names=TRUE)
...
for (f in simDatFiles) {
	...
	res <- foreach(batch=1:ncores, .combine=rbind) %dopar% {    
		...
		for (i in 1:n.MA.piece) {
			...
			res0 <- rbind(
				...
				pc_skew(t=MAdat$t, df=MAdat$n1 + MAdat$n2 - 2, long=TRUE),
				pcurveEst(t=MAdat$t, df=MAdat$n1 + MAdat$n2 - 2, progress=FALSE, long=TRUE, CI=FALSE),
	...
	save(res, file=paste0("analysisParts10000/analysis_", basename(f)), compress="gzip")
} # of "f in simDatFiles"
