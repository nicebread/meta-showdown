...
# Figure for blog post

summ %>% 
  filter(method %in% c("reMA", "TF"),
         tau == 0, qrpEnv == "none") %>% 
  ggplot(aes(x = delta, y = meanEst, col = method)) +
  geom_point() +
  facet_wrap(~selProp)

summ %>% 
  filter(method %in% c("reMA", "TF"),
         tau == 0, qrpEnv == "none", k == 100) %>% 
  ggplot(aes(x = factor(selProp), y = meanEst, col = method)) +
  geom_point() +
  facet_wrap(~delta.label) +
  geom_hline(aes(yintercept = delta)) +
  scale_y_continuous("Mean estimate") +
  scale_x_discrete("Degree of publication bias", labels = c("0%", "60%", "90%"))
