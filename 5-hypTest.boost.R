...
save(res.hyp, file="res.hyp.boost.RData", compress="gzip")
#load(file="res.hyp.RData")

# ---------------------------------------------------------------------
# Compute summary measures across replications
hyp.summ <- res.hyp %>% group_by(condition, k, k.label, delta, delta.label, qrpEnv, qrp.label, method) %>% dplyr::summarise(
...
hyp.summ <- hyp.summ %>%  mutate(loop = paste0(k))
...
RR <- hyp.summ %>% ungroup() %>% 
	select(condition, k, delta, qrp.label, loop, method, H0.reject)
...
H1 <- 0.5
RR.H0 <- RR %>% filter(delta == 0) %>% select(condition, k, qrp.label, loop, method, TypeI)
RR.H1 <- RR %>% filter(delta == H1) %>% select(k, qrp.label, loop, method, Power)
...
ggplot(RR.wide, aes(x=loop, y=log(rejectionRatio), group=loop)) + 
	geom_point() + 
...
wrongSig <- res.wide.red %>% select(1:8, b0_estimate, b0_p.value) %>% 
	...
	group_by(condition, k, delta, qrpEnv, method) %>% 
	summarise(
		wrongSig = sum((b0_estimate < 0) & (b0_p.value < .05)) / n()
	) %>% 
	mutate(loop = paste0(k))
...
#+ echo=FALSE, fig.width=11, fig.height=8, warning=FALSE
ggplot(wrongSig, aes(x=loop, y=wrongSig, color=factor(delta))) + geom_point() + facet_grid(qrpEnv ~ method) + theme(axis.text.x = element_text(angle = 90, size=6, hjust=1, vjust=.5)) + ylab("Percentage of significant estimates in wrong direction")
