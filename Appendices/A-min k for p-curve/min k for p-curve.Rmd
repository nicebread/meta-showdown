...
# estimation
res.est <- res.wide %>% filter(method %in% c("pcurve", "puniform")) %>% dplyr::select(1:9, kSig_estimate, 41:45)

# hypothesis test
res.hyp <- res.wide %>% filter(method %in% c("pcurve", "pcurve.evidence", "puniform")) %>% dplyr::select(1:9, b0_p.value, skewtest_p.value, kSig_estimate, 41:45)
...
summ.est <- res.est %>% 	
	group_by(delta, delta.label, qrpEnv, qrp.label, selProp, selProp.label, tau, tau.label, method, kSig_estimate) %>% 	
...
summ.hyp <- res.hyp %>%  
	...
	group_by(delta, delta.label, qrpEnv, qrp.label, selProp, selProp.label, tau, tau.label, kSig_estimate) %>% 
	dplyr::summarise(
		n = n(),
...
summ.hyp.long <- melt(summ.hyp, id.vars=c("delta", "delta.label", "qrpEnv", "qrp.label", "selProp", "selProp.label", "tau", "tau.label", "kSig_estimate", "n"))
...
```{r echo=FALSE}
ggplot(summ.est %>% filter(kSig_estimate <= 10, n>=min.simulations, selProp==0), aes(x=kSig_estimate, y=RMSE, color=factor(delta), shape=method)) + geom_point() + geom_line(aes(group=factor(delta):factor(method))) + geom_hline(yintercept=0, linetype="dotted") + facet_grid(qrp.label~tau.label) + theme_bw()+ xlab("# of significant studies entered in method") + scale_x_continuous(breaks=c(2, 4, 6, 8, 10))
```
...
```{r echo=FALSE}
ggplot(summ.hyp.long %>% filter(kSig_estimate <= 15, n>=min.simulations, delta==0, selProp==0), aes(x=kSig_estimate, y=value, shape=variable, color=variable)) + geom_point() + geom_line(aes(group=variable)) + geom_hline(yintercept=0.05, linetype="dotted") + facet_grid(qrp.label~tau.label) + theme_bw() + ylab("False positive (Type I) error rate") + xlab("# of significant studies entered in method") + ggtitle("Type I error rate for 0% publication bias")
```

## True positive rate (statistical power) for $\delta$ = 0.2

```{r echo=FALSE}
ggplot(summ.hyp.long %>% filter(kSig_estimate <= 15, n>=min.simulations, delta==0.2, selProp==0), aes(x=kSig_estimate, y=value, shape=variable, color=variable)) + geom_point() + geom_line(aes(group=variable)) + geom_hline(yintercept=0.80, linetype="dotted")  + facet_grid(qrp.label~tau.label) + theme_bw() + ylab("Power") + xlab("# of significant studies entered in method") + ggtitle(bquote("Power for "~delta~" = 0.2 and 0% publication bias"))
```