...
res.wide <- res.final %>% 
	filter(term %in% c("b0"), method %in% c("reMA", "TF", "WAAP-WLS", "pcurve", "puniform", "PETPEESE.lm", "3PSM")) %>% 
...
summ <- res.wide %>% group_by(condition, simBatch, k.label, delta, delta.label, censor.label, tau.label, method) %>% dplyr::summarise(
...
ggplot(summ, aes(x=method, y=meanEst, color=simBatch, shape=factor(delta.label))) + geom_point() + geom_hline(aes(yintercept=delta), linetype="dotted") + facet_grid(k.label~tau.label~censor.label) + coord_flip(ylim=c(-0.5, 1))
```
