...
gradient <- function(SEREadjustedest, y, v, p) {
   ...
   return( matrix(c(dlda,dldb,dldvc,dldmu),1,4) ) }

#3) Calculate the hessian.

SERE.hessian <- function(SEREadjustedest, y, v, p) {
   a2 <- SEREadjustedest[1]
   b2 <- SEREadjustedest[2]
   vc2 <- SEREadjustedest[3]
   mu2 <- SEREadjustedest[4]
   ...
   return( matrix(c(d2lda2,d2ldadb,d2ldadvc,d2ldadmu,
                    d2ldadb,d2ldb2,d2ldbdvc,d2ldbdmu,
                    d2ldadvc,d2ldbdvc,d2ldvc2,d2ldvcdmu,
                    d2ldadmu,d2ldbdmu,d2ldvcdmu,d2ldmu2),4,4) ) 
}
