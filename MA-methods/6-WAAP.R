...
WAAP.est <- function(d, v, n1, n2, est=c("WAAP-WLS"), adequate.power=0.80, long=TRUE) {
	...
	power <- pwr.t2n.test(n1 = n1, n2= n2, d = abs(true.effect), sig.level = 0.05, alternative = c("two.sided"))$power
	...
	kAdequate <- sum(power >= adequate.power)
	
	if (kAdequate >= 2) {
		res <- WLS.est(d[power >= adequate.power], v[power >= adequate.power], long=FALSE)
		res$method <- "WAAP-WLS"
		
		res <- plyr::rbind.fill(res, 
			data.frame(method="WAAP-WLS", term="estimator", type="WAAP", kAdequate=kAdequate)
		)
		...
		res <- plyr::rbind.fill(res, 
			data.frame(method="WAAP-WLS", term="estimator", type="WLS", kAdequate=kAdequate)
		)
	}
