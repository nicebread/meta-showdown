# ---------------------------------------------------------------------
#  Estimate the weight-function model implemented in McShane et al. 2016
#' @param long Should the results be returned in long format?

McShane.est <- function(t, n1, n2, long=TRUE) {	
	...
	if(!is.null(mm)){
		...
		res.wide <- data.frame(
			...
			std.error = SE,
			statistic = NA,
			p.value = pnorm(mm[[1]][1] / sqrt(mm[[2]][1,1]), lower.tail=FALSE)*2,
...
# dat <- data.frame(dat)
# McShane.est(t=dat$t, n1=dat$n1, n2=dat$n2)